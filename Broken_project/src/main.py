...
def iniciar_sesion():
    ...
    # Lógica de autenticación (esto debe implementarse en UsuarioDAO)
    usuario_dao = UsuarioDAO()
...
def listar_activos(usuario):
    ...
    for accion in acciones:
        print(f"Nombre: {accion.nombre}, Cantidad: {accion.cantidad}, Precio Actual: {accion.precio_actual}")
...
def comprar_acciones(usuario):
    ...
    if accion and cantidad > 0:
        ...
        if usuario.saldo >= total_precio:
            ...
            transaccion.create(usuario.usuario, accion.accion_id, 'compra', cantidad, accion.precio_compra_actual)
